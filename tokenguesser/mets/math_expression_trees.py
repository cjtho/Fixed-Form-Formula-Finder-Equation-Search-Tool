...
class FunctionSampler:
    def __init__(self, complexity: float = 0.0, alpha: float = 0.5):
    ...
    def _compute_probabilities(self) -> Dict[str, float]:
        complexities = [defn["complexity"] for defn in self.function_definitions.values() if defn.get("complexity")]
        min_c, max_c = min(complexities), max(complexities)

        # Normalize complexities with alpha smoothing
        normalized_complexities = [(max_c - c + self.alpha) / (max_c - min_c + len(complexities) * self.alpha) for c in
                                   complexities]

        # Create a reversed distribution for bias towards complex functions, including alpha in the calculation
        reversed_distribution = [1 - n for n in normalized_complexities]

        # Interpolate based on the complexity parameter to shift bias from simple to complex functions
        interpolated = [linear_interpolate(n, r, self.complexity) for n, r in
                        zip(normalized_complexities, reversed_distribution)]

        # Ensure total of interpolated values is > 0 before proceeding to normalization
        if sum(interpolated) == 0:
            interpolated = [i + self.alpha for i in interpolated]

        total_interpolated = sum(interpolated)
        # Normalize interpolated values to sum to 1 (probabilities)
        probabilities = {name: i / total_interpolated for name, i in
                         zip(self.function_definitions.keys(), interpolated)}
        return probabilities

    def get_function_of_type(self, return_type: str) -> MathematicalFunction:
        candidates = {name: self.probabilities[name] for name, defn in self.function_definitions.items()
                      if defn["output_input_type_map"].get(return_type) and defn.get("arity")}
        total_probability = sum(candidates.values())
        normalized_probabilities = [candidates[name] / total_probability for name in candidates]
        chosen_function_name = random.choices(list(candidates.keys()), weights=normalized_probabilities)[0]
...
def digit_tree(number: Union[List[int], int]) -> Node:
    ...
    if is_negative:  # Construct -1 * (BLANK)
        root.set_math_func(get_function("MULT"))
        left_child = Node()

        # 0 - 1 = -1
        left_child.set_math_func(get_function("SUB"))
        left_left_child = Node()
        left_left_child.set_math_func(digit_tree(0))
        left_right_child = Node()
        left_right_child.set_math_func(digit_tree(1))
        left_child.set_children([left_left_child, left_right_child])
...
def generate_sequence_function_pair(visualize: bool = False,
    ...
    function_handler = FunctionSampler(func_complexity)
    ...
    while not solution_found:
        ...
        try:
            outputs = tree.validate_tree(inputs)
